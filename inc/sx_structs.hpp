...
/*   Updated: 2015/05/31 00:53:31 by hestela          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef SX_STRUCTS_HPP
# define SX_STRUCTS_HPP
...
winPtr: (SxWindow*)pWindow;
- (void)putPixel: (NSPoint)pixel
color: (int32_t)rgb;
- (void)blitSurface: (uint32_t*)surface
size: (NSSize)size
to: (NSPoint)place;
- (void)clear;
- (void)updateContext;
...
struct						s_sx_win
{
	...
	void					(*push_event)(t_sx_win *sx_win_p, t_event_pool *e);
	int						gl;
	void					*next;
};
